...
static int acquire_lock(void) {
    const int fd = open("/tmp/ensure_role_and_database_exists.lock", O_CREAT | O_RDWR, 0600);

    if (fd == -1) {
        ereport(ERROR, errmsg("could not open lock file: %m"));
    }

    if (flock(fd, LOCK_EX) == -1) {
        close(fd);
        ereport(ERROR, errmsg("could not acquire lock: %m"));
    }

    return fd;
}

static void release_lock(const int fd) {
    if (flock(fd, LOCK_UN) == -1) {
        elog(WARNING, "could not unlock lock file: %m");
    }
    ...
    PG_TRY();
        {
            ...
            if (strcmp(port->user_name, postgres_user) != 0) {
                elog(LOG, "handling connection for username '%s' to database '%s'", port->user_name,
                     port->database_name);
            ...
            if (strcmp(port->database_name, postgres_user) != 0) {
                ...
                const char *database_owner = (database_owner_env && database_owner_env[0] != '\0')
                                                 ? database_owner_env
                                                 : port->user_name;
                ...
                {
                    // if enabled, grant postgres v14 and earlier style permissions of create on schema public, for apps that rely on it.
                    ...
                    if (grant_public != NULL && strcmp(grant_public, "true") == 0) {
                        ...
                        if (asprintf(
                                &cmd,
                                "PGOPTIONS='-c ephemeral_pg.internal=true' psql -U %s -d %s -c \"GRANT CREATE ON SCHEMA public TO PUBLIC\"",
                                postgres_user, port->database_name) < 0) {
                            ereport(ERROR, errmsg("failed to allocate command string"));
                        }
